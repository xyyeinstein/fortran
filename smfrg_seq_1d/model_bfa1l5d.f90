subroutine BaFeAs1L5d()
  ...
  interface
    subroutine CheckHopBaFeAs1L5d(norb,model)
  ...
  kmeshfile='BFA1L5dkmesh.dat'
  useprojectors=.false.; appendprojectors=.false.; projectorfile='BFA1L5dproj.dat'
  ...
  ! this is a test of supercell method, and should be compared to the 5-orbit model
    ...
    model%norb=norb; model%natom=natom; model%sample='BFA1L5d'
    ...
    allocate(model%orbits(norb)); model%orbits(1:5)=(/'A1g','E1g','E2g','B2g','B1g'/)
	...
	!--------------------------------------------------------------------------------
	model%U=1.00; model%JH=0.25; model%Uab=model%U-2*model%JH; 

	open(10,file='BaFeAs1L5d.model')
	  read(10,*)model%t
	close(10)
    
    nband=3; model%nband=nband; model%mu=11.5  !9.48 for LiFeAs
    allocate(model%npatch(nband),model%nativeband(nband),model%pocket(nband),model%mushift(nband))
    model%nativeband=(/2,3,4/);	model%pocket=(/'M_','GM','XY'/); model%npatch=(/32,64,64/)*2; model%mushift=0.
    !------------------------------------------------------------------------------------

	Q=0
    call BasisRotateBFA1L5d(norb,model)
	!call FindMaxHopLFA2L5d(norb,model); pause
	call CheckHopBaFeAs1L5d(norb,model)
...
subroutine BaFeAs1L5dhk(norb,kv,Q,hk,model,mf,slvbsn)
  use standard_derived_types
  implicit none
  integer norb
  real*8 kv(2),Q
...
subroutine BasisRotateBFA1L5d(norb,model)
  use standard_derived_types
  implicit none
...
subroutine FindMaxHopBFA1L5d(norb,model)
  use standard_derived_types
  implicit none
