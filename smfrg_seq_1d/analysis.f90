subroutine FermiSurf(norb,Q,model)
  ...
  scale=1; if( abs( sum(model%a*model%b) )>1.d-5 )scale=1.4
...
subroutine channel(name,q,nA,A,Smax,isave,iXq)
  ...
  B=A
  call ZSVD( nA,nA,B,U,Sig,V )
  do iS=1,nA
	Si(iS)=Sig(iS)*real( sum( U(:,iS)*V(iS,:) ) )
  end do
  Si=-Si
  call ascendingorder(nA,Si,info); Si=-Si
  Smax=Si(1) 
  if(.not.isave)return
  ...
  do idim=1,nA
	...
	U(idim,:)=U(idim,:)*phase
    V(:,idim)=V(:,idim)*conjg(phase)
  end do
  
  if(name=='C')open(10,file='sdwMode.txt')
  if(name=='D')open(10,file='cdwMode.txt')

  write(10,*)'  ordering momentum/pi='; write(10,*)q
  write(10,*)' '
  do iS=1,nA
    if(abs(Sig(iS))<1.e-10)cycle   !too small eigenvalue not saved
    iSS=info(iS)
	write(10,*)'   iS, S =',iS,Si(iS)
    write(10,*)'   Mform    Lform     rbasis       Oform           Uvec               Vvec'
	do idim=1,nA; 
	  if(abs(U(idim,iSS))+abs(V(idim,iSS))<1.e-4)cycle  !irrelevant forms not saved
...
200 format(1x, i4, i3, ' ', a3, ' L-', a3, 3f5.1, ' O-', a3,a1, ' ', 2i2, 4f8.4)
  ...
  if(name=='C')then
	  open(11,file='sdwmode.dat')
	  open(12,file='sdwform.dat')
  end if
  if(name=='D')then
	  open(11,file='cdwmode.dat')       
	  open(12,file='cdwform.dat')
  endif
  write(11,*)q
...
subroutine pairchannel(nA,Smax,isave)
  ...
  open(10,file='pairMode.txt')
  open(11,file='pairmode.dat')
  open(12,file='scform.dat')  
  do imode=1,10
...
subroutine bubbleSusceptibility()
  ...
  G=0; M=(/1,1/); X=(/1,0/)
  hexagonal=( abs(sum(model%a*model%b))>1.d-5 )
  if(hexagonal)then
  ...
  do iq=1,mq
    do ik=1,mk
      ...
      do ia=1,nambu; do ib=1,nambu
        ...
        Linhard=( fermi(ea,1.d-3)-fermi(eb,1.d-3) ) / ( v+one*eta - (ea-eb) )
...
subroutine BandStructure(norb,Q,model)
  ...
  if(sum(abs(model%kb))==0)then
	...
	close(10)
    print*,'1d band structure output to file'
    return
  end if
  ...
  do ik=1,mk
    !BdG eigenstates at k
     ...
     do ia=1,nambu; do ib=1,nambu             !sum over band indices
		...
		Linhard=( fermi(ea,1.d-3)-fermi(eb,1.d-3) )*k(3,ik) / ( v+one*eta - (ea-eb) )
