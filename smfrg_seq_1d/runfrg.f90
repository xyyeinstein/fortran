subroutine runfrg()
  ...
  complex*16,dimension(ndim) :: Usc,Usdw,Ucdw
  integer phase
  ...
  do while(w>=wir)
    ...
    La=w*(1+1/b)*0.5
    dLa=w*(1-1/b)
    if(.not.uniformkmesh)call flexiblemesh(La)
...
230 format(1x,f15.6,2(f15.6,a6))
  ...
  open(10,file='scform.dat')
  open(11,file='sdwform.dat')
  ...
  close(11)
  close(12)
  if(output%Vmax(2)>output%Vmax(3).and.output%Vmax(2)>output%Vmax(4))then
    if(abs(Usc(1))>0.8)phase=1
    if(abs(Usc(1))<0.1)phase=2
  end if
  if(output%Vmax(3)>output%Vmax(2).and.output%Vmax(3)>output%Vmax(4))phase=3
  if(output%Vmax(4)>output%Vmax(2).and.output%Vmax(4)>output%Vmax(3))then
    if(abs(Ucdw(1))>0.8)phase=4
    if(abs(Ucdw(1))<0.8)phase=5
  end if
  write(30,'(4f10.5,1i4)')model%U,model%Vnn,model%mu,w,phase
  write(*,'(4f10.5,1i4)')model%U,model%Vnn,model%mu,w,phase
