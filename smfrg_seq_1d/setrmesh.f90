subroutine setRmesh()
  ...
  Lcontacty=Lcontact
  if(sum(abs(model%kb))==0)Lcontacty=0
  iR=0
  do iy=-Lcontacty,Lcontacty 
  do ix=-Lcontact,Lcontact
  ...
  do iR=1,nR
     if(findR(R(:,iR))<0)then
	stop 'R is not searchable @ setRmesh'
     end if
  end do
  ...
  do jatom=1,natom
    ...
    do ig=1,ng
      call groupaction(ig,rv,rg,group)
	  iag=atomimage(ig,iatom,model)
	  jag=atomimage(ig,jatom,model)
	  rg=rg-ra(:,jag)+ra(:,iag)
	  ix=nint( sum(rg(1:2)*model%ka) / 2 )
	  iy=nint( sum(rg(1:2)*model%kb) / 2 )
	  if(sum( abs(rg-ix*model%a-iy*model%b) )>1.e-5 )then
	    print*,'rg,ix,iy',rg,ix,iy
	    stop 'Rg not truncatable @ setRmesh'
	  end if
	  iRg=findR( (/ix,iy/) )
