subroutine LiFeAs1L10d()
  ...
  interface
    subroutine CheckHopLiFeAs1L10d(norb,model)
  ...
  ! this is a test of supercell method, and should be compared to the 5-orbit model
    ...
    norb=10; natom=2
    model%norb=norb; model%natom=natom; model%sample='LFA1L10d'
    print*,'The sample chosen is ',model%sample 

	allocate(model%ra(3,natom)); model%ra(:,1)=(/0.0,0.0,0.5/); model%ra(:,2)=(/0.5,0.5,0.5/)
	...
	nbondxy=9
	nbondz=2
	open(10,file='LiFeAs1L10d992.model')
    !open(10,file='NaFeAs10dmodel')
	...
	do while (.true.)
      ...
      ix=nint(x); iy=nint(y); iz=nint(z)
      model%t(a,b,ix,iy,iz)=tt
      if(error/=0)exit
    end do
	close(10)
    call BasisRotateLiFeAs1L10d(norb,model)
	call CheckHopLiFeAs1L10d(norb,model)
    
    
	nband=4; model%nband=nband; model%mu=9.39
...
subroutine LiFeAs1L10dhk(norb,kv,Q,hk,model,mf,slvbsn)
  use standard_derived_types
  implicit none
  integer norb
  real*8 kv(2),Q
...
subroutine BasisRotateLiFeAs1L10d(norb,model)
  use standard_derived_types
  implicit none
