...
subroutine gethk(norb,kv,Q,hk,model,mf,slvbsn)
  ...
  select case (norb)
    case (1)
	  if(model%sample=='sq1a1d')call square1a1dhk(norb,kv,Q,hk,model,mf,slvbsn)
	  ...
	  if(model%sample=='ehm1d')call ehm1dhk(norb,kv,Q,hk,model,mf,slvbsn)
    case (2)
	...
	case (5)
	  if(model%sample=='WangFa')call WangFa5dhk(norb,kv,Q,hk,model,mf,slvbsn)
    ...
    case (20)
      call KFeSe2L20dhk(norb,kv,hk,model,mf,slvbsn)
	case default
	  stop 'model hk unknown'
  end select
  return
