subroutine search(px,ipx,mesh,uniform)
  ...
  if(sum(abs(px))<1.e-10)then
    ipx=mesh%npoints+1
    return
  end if
  
  if(sum(abs(model%kb))==0)then
    call search_1d(px,ipx,mesh)
	return
  end if
...
subroutine search_1d(px,ipx,mesh)
  use standard_derived_types
  implicit none
  type (meshconfig) :: mesh 
  real*8 px(2)
...
subroutine BZmesh1d(ng,wmax,wmin,mesh,epfunc)
  use standard_derived_types
  implicit none
  integer ng,nmesh
  real*8 wmax,wmin
  ...
  do while(w>=wmin)
     ...
     do i=1,np; pc=mesh%g(ig)%p(:,i); expand=.false.
        do ix=-nsearch,nsearch
           ep= epfunc(pc+(/ix,0/)*dp*2./nsearch) 
           if(abs(ep)<=w)expand=.true.; if(expand)exit
        end do	     
        if(.not.expand)cycle
        mesh%g(ig)%info(i)=icount+1
     ...
     mesh%g(ig+1)%np=icount; mesh%g(ig+1)%dp=dp; mesh%g(ig+1)%area=dp; mesh%g(ig+1)%w=w
     allocate(mesh%g(ig+1)%info(icount),mesh%g(ig+1)%p(2,icount))
     mesh%g(ig+1)%info=0
     ...
     do i=1,np; if(mesh%g(ig)%info(i)==0)cycle; pc=mesh%g(ig)%p(:,i)
        do ix=-1,1,2
				   icount=icount+1
           mesh%g(ig+1)%p(:,icount)=pc+(/ix,0/)*dp
        end do	  
     end do
