subroutine SrRuO2orb()
  ...
  BCSFlow=.false.
  Appendkmesh=.true.; kmeshfile='SRO2orbkmesh.dat'
  useprojectors=.false.

    norb=2; natom=1
    model%norb=norb; model%natom=natom; model%sample='SRO2orb'
    print*,'The sample chosen is ',model%sample; 

	allocate(model%ra(3,natom));  model%ra=0
    allocate(model%orbits(norb)); model%orbits=(/'E1g','E2g'/)
	model%periodiclayers=.false.
	
	usegroup=.true.;  group='C4v';  ng=8
    model%group=group; model%ng=ng

    model%U=4; model%JH=0.5; model%Uab=model%U-2*model%JH; model%mu=1
	nband=2; model%nband=nband
	allocate(model%npatch(nband),model%nativeband(nband),model%pocket(nband),model%mushift(nband))
	model%nativeband=(/1,2/); model%pocket=(/'G_','M_'/); model%npatch=(/96,96/); model%mushift=0.
	...
	call writeinputs()
	call setupworkplace()
	cutoff=1.e-2; call rpa(cutoff,appendkmesh,'SRO2dX0.dat'); stop
...
end subroutine SrRuO2orb


subroutine SrRuO2orbhk(norb,kv,hk,model,mf,slvbsn)
