...
subroutine setgroupindex()
  ...
  pv=0; info=-1
  do iq=1,nq
    if(abs(qv(3,iq))<1.e-10)then; info(iq)=iq; indexqg(:,iq)=iq; cycle; end if
	  pv(1:2)=qv(1:2,iq); active=.not.reducible(pv(1:2),group)
    if(active)info(iq)=iq
    do ig=1,ng; call groupaction(ig,pv,pg,group); iqg=findq(pg(1:2),hexagonal)
	  call rbzkv(pg(1:2)-qv(1:2,iqg),model%a,model%b,qq)
		...
		if(active)info(iqg)=iqg; indexqg(ig,iq)=iqg
     end do
  end do
...
function findq(q,hexagonal)
  ...
  if(model%ladder)then
    error=1.e10
    do iq=1,nq; call rbzkv(q-qv(1:2,iq),model%a,model%b,qq)
	   dist=sum( abs(qq) ); if(dist<error)then; error=dist; findq=iq; end if
	end do
  else
    if(hexagonal)then; call hexsearch(q,findq,qmesh); else; call search(q,findq,qmesh,uniformqmesh); end if
  end if
  return
end function findq
